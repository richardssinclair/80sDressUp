...
class Person {
    ...
    //         clothesbutton.setAttribute('src', data[b])
    //         document.querySelector('.character-wardrobe__elements').appendChild(clothesbutton);
    //         clothesbutton.addEventListener('click', (e) => {
    //             this.dressCharacter(e, parentTarget)
    //         });
    //     }
    // }
    ...
    addEvents = (div) => {
        div.addEventListener('click', (event)=>{
            ...
            // Clear character cothes before they are selected...(will interfear with state)
            clearCharacter();
            ...
            document.querySelector('#character-description-id').innerHTML += `<h1 class="character-name">${this.state.name}</h1>`
